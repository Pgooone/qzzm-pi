...
export function buildPrdTools(projectId: string) {
  ...
  const readPrd = defineTool({
    name: "read_prd",
    label: "读取当前 PRD",
    description: "读取 artifacts/prd.md。可指定版本号读取历史版本，不指定则读最新。",
    parameters: Type.Object({
      version: Type.Optional(Type.String({ description: "版本号 final / v1 / v2，不填则读最新" })),
    }, { additionalProperties: false }),
    execute: async (_id, params) => {
      try {
        const content = await artifactStore.load(projectId, "prd.md", params.version || undefined)
